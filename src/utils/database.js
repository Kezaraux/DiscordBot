...
if (config.get("features.reactRoles")) {
  ...
  if (!messageTable["count(*)"]) {
    ...
    sql
      ...
      .run();
    sql
      .prepare("CREATE UNIQUE INDEX idx_messages_id on reaction_messages (id);")
      .run();
    sql.pragma("synchronous = 1");
    sql.pragma("journal_mode = wal");
  ...
  getReactionRolesForMessage = sql.prepare(
...
if (!configTable["count(*)"]) {
  ...
  if (!configTable["count(*)"]) {
    ...
    sql
      ...
      .run();
    sql
      .prepare("CREATE UNIQUE INDEX idx_config_id on guild_config (guild_id);")
      .run();
    sql.pragma("synchronous = 1");
    sql.pragma("journal_mode = wal");
...
export const getGuildConfig = sql.prepare(
