...
const setupInitialGuildConfig = (db, guildId, initialConfig) => {
    ...
    if (!tableExists["count(*)"]) {
        ...
        if (!configTable["count(*)"]) {
            ...
            db.prepare("CREATE TABLE guild_config (guild_id TEXT PRIMARY KEY, config TEXT);").run();
            db.prepare("CREATE UNIQUE INDEX idx_config_id on guild_config (guild_id);").run();
        ...
        db.prepare("INSERT OR REPLACE INTO guild_config (guild_id, config) values (@guild_id, @config)").run({
            guild_id: guildId,
